...
namespace BigFileUpload.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Upload()
        {
            ...
            var id = Url.Encode(Request["id"]);//当前文件id
            var dir = Server.MapPath("~/Upload");//文件上传目录
            dir = Path.Combine(dir, guid , id);//临时保存分块的目录
        ...
        public ActionResult Merge()
        {
            ...
            var name = Request["name"];//文件名
            var id = Url.Encode(Request["id"]);//当前文件id
            var dir = Path.Combine(uploadDir, guid, id);//临时文件夹
            var files = System.IO.Directory.GetFiles(dir);//获得下面的所有文件
            var finalPath = Path.Combine(uploadDir, name);//最终的文件名（demo中保存的是它上传时候的文件名，实际操作肯定不能这样）
        ...
        public ActionResult AllFinish()
        {
            var guid = Request["guid"];//GUID
            var uploadDir = Server.MapPath("~/Upload");//Upload 文件夹
            var dir = Path.Combine(uploadDir, guid);//临时文件夹

            System.IO.Directory.Delete(dir,true);//删除文件夹
